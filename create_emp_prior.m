...
for i = 1 : size(train_pos_vecs, 1)
    ...
    for j = 1 : num_predicates
        if j <= num_features_to_learn   % this is a high predicate
            muvec = high_mus(:, :, j); 
            sigma_inv_mat = high_inv_sigmas(:, :, j);
        else    % this is a low predicate
        ...
        % Increase the vote for the predicate (for role 1) if it is more
        % likely to apply to animal1 than to animal2
...
if length(best_predicates) > 1
    ...
    for pind = 1:length(best_predicates)
        if best_predicates(pind) <= num_features_to_learn     % this winning predicate is a high predicate
            ...
            % Add that low predicate for role 1 and subtract it for role 2
    ...
    if best_predicates <= num_features_to_learn     % the winning predicate is a high predicate
        ...
        % Use that low predicate for role 1 and reverse signs for role 2
        mu_role1 = low_mus(:, :, best_predicates - num_features_to_learn);
        mu_role2 = -mu_role1; 
    end
end
